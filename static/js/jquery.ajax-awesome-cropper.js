...
(function() {
  ...
  $.ajaxAwesomeCropper = function(inputAttachTo, options) {
    ...
    sendAjax = function(img) {
      console.log(img);
      $.ajax({
        type: 'POST',
        url: url.ajax_upload,
        data: {image64: img, project_id: info.project_id},
  ...
  */
  ...
  $.fn.extend({
    ajaxAwesomeCropper: function(options) {
      return this.each(function() {
        if ($(this).data("ajaxAwesomeCropper")) {
          if (options.remove) {
            $(this).data("ajaxAwesomeCropper").remove();
            $(this).removeData("ajaxAwesomeCropper");
          } else {
            $(this).data("ajaxAwesomeCropper").setOptions(options);
          }
        } else if (!options.remove) {
          $(this).data("ajaxAwesomeCropper", new $.ajaxAwesomeCropper(this, options));
        }
        if (options.instance) {
          return $(this).data("ajaxAwesomeCropper");
        }
        return this;
      });
    }
