...
def brainstorm():
    ...
    # notes = db( (BrainstormNotes.project_id==project_id) ).select()
    # notes_with_group = db( (BrainstormNotes.project_id==project_id) & \
    #     (BrainstormNotes.group_id!=None) ).select(BrainstormNotes.ALL, orderby=BrainstormNotes.group_id ,groupby=BrainstormNotes.group_id)

    person = db(User_relationship.auth_user_id==auth.user.id).select().first()

    #TODO: pegar os grupos tudo
    #TODO: pegar as notas normais verificando se o id delas esta na query dos grupos

    groups = db(BrainstormRelationsNotesGroups.id>0).select()
    print dir(groups)
    print groups.records# -*- coding: utf-8 -*-
# this file is released under public domain and you can use without limitations
...
def brainstorm():
    project_id = request.args(0) or redirect('projects')
    person = db(User_relationship.auth_user_id==auth.user.id).select().first()
    groups = db(BrainstormGroups.project_id==project_id).select()

    lista_notas_com_grupo = []
    for group in groups:
...
def burndown_chart_test(story_project_id, definition_ready_story, definition_was_concluded=False):
    ...
    else: # chamado quando o definition_ready_story == False
        sprint_ = db((Sprint.project_id==story_project_id) & (Sprint.ended==None)).select(orderby=Sprint.started).first()
        stories = db((Story.project_id == story_project_id) & (Story.sprint_id == sprint_.id)).select()
        story_points_sprint = sprint_.story_points
...
def _test_story_completed(definition_ready_id, status):
    ...
    if status == "done" or status == "remove":
        ...
        # add points in the date of sprint
        burndown_chart_test(story.project_id, False, definition.concluded)

        return False


    # notesA = db(BrainstormNotes.project_id==project_id).select()



    return dict(notes=notes,
                notes_with_group=notes_with_group,
                project_id=project_id,
                person_id=person.person_id)
